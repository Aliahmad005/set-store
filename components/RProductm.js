...
const RProductm = () => {
  ...
  return (
    <div className='mt-[70px] tablet:ml-20 laptop:ml-4 desktop:ml-4 tablet:mr-2 laptop:mr-4 desktop:mr-4 md:mt-[100px]  md:mb=0'>
      ...
      {loading === "false" ?
        <Carousel draggable={false} swipeable={false} responsive={responsive} containerClass='-mx-[10px]  ' itemClass="px-[10px]" >
          {topics.map((data, i) => {
            return <div key={i}  className=' w-fit cursor-pointer '>
              <Image onClick={()=>nav(data._id)} className='m-auto'
                src={data.img}
                width={400}
                height={300}
