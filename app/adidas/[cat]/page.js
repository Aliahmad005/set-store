...
const [adr , setAdr] = useState([{name:1},{name:1},{name:1},{name:1},{name:1},{name:1},{name:1},{name:1},]);
  ...
  const getTopics = async (e) => {
    console.log("checking", e)
     try {
       const res = await fetch(`/api/getcat/${e}`, {
         cache: "no-store",
       });
       console.log("data chul rha hai" , res)
       if (!res.ok) {
...
<form className="flex items-center w-2/3">   
    ...
    <div className="relative w-full">
        ...
        <input type="text" id="voice-search" ref={sea} onChange={search} className=" border border-gray-900 text-gray-900 text-sm rounded-full focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search here" required />
    ...
    <button type="submit" className="inline-flex items-center text-gray-900 py-2.5 px-3 ml-1 text-sm font-medium  rounded-lg    focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
               ...
               
{
  catt.map((data , i)=>{
    return    <p key={i} onClick={()=>router.push(`/adidas/${data.cat}`)} className='productPageLCatMenu' >{data.cat}</p>
        ...
        <input type="number" id="voice-search"  ref={min} className=" border border-gray-900 text-gray-900 text-sm rounded-lg  focus:ring-blue-500 focus:border-blue-500 block ml-3 w-3/4  pl-10 p-2  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Min Price" required />

                    <p  className='productPageLCatMenu pt-3' >max Price</p>
        <input type="number" id="voice-search"  ref={max} className=" border border-gray-900 text-gray-900 text-sm rounded-lg  focus:ring-blue-500 focus:border-blue-500 block ml-3 w-3/4  pl-10 p-2  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Min Price" required />
                  

                    <h5 onClick={filter} className='productPageLCatHeading text-emerald-800 cursor-pointer'>Filter</h5>
